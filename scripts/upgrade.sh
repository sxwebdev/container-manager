...
TEMP_DIR="/tmp/container-manager-upgrade-$$"
...
usage() {
    ...
    echo "  $0 container-manager ./"
    echo "  $0 container-manager-prod /opt/container-manager/bin/container-manager"
    echo ""
...
if ! systemctl list-unit-files | grep -q "^$SERVICE_NAME.service"; then
    ...
    print_warning "Available services containing 'container-manager':"
    systemctl list-unit-files | grep -i container-manager || true
    exit 1
fi
...
print_success "All done! Container Manager has been upgraded to $NEW_VERSION"
